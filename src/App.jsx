...
import heroPoster from './assets/Foto/hero-poster.jpg';
import heroVideo from './assets/Video/hero-video.mov';
import featureImg1 from './assets/Foto/askold-island.png';
import featureImg2 from './assets/Foto/hero-poster.jpg';
import featureImg3 from './assets/Foto/three-lakes.png';
import captainImg from './assets/Foto/captain-semen.jpg';
import yachtImg from './assets/Foto/hero-poster.jpg';
import protocolImg1 from './assets/Foto/askold-island.png';
import protocolImg2 from './assets/Foto/lisiy-island.jpg';
import protocolImg3 from './assets/Foto/three-lakes.png';
import galleryImg1 from './assets/Foto/lisiy-island.jpg';
import galleryImg2 from './assets/Foto/gallery-1.jpg';
import galleryImg3 from './assets/Foto/askold-island.png';
import galleryImg4 from './assets/Foto/gallery-3.jpg';
import galleryImg5 from './assets/Foto/gallery-4.jpg';
import galleryImg6 from './assets/Foto/gallery-2.jpg';
    ...
    <header className={`fixed top-0 left-0 w-full z-[150] transition-all duration-500 ${scrolled ? 'py-4' : 'py-8'}`}>
      <div className={`max-w-7xl mx-auto px-6 md:px-8 py-4 rounded-full transition-all duration-500 flex justify-between items-center text-primary ${scrolled ? 'bg-dark/95 backdrop-blur-xl shadow-2xl border border-white/10 mx-4 md:mx-auto' : 'bg-dark/40 backdrop-blur-md border border-white/10 mx-4 md:mx-auto'}`}>
        ...
        <nav className="hidden lg:flex items-center gap-10 font-heading text-[14px] lowercase tracking-[0.15em] font-medium text-white/90 px-6">
          ...
          <button onClick={() => scrollTo('protocol')} className="hover:text-accent transition-colors shrink-0">маршруты</button>
      ...
      {isOpen && (
        <div className="lg:hidden fixed inset-0 bg-dark/98 backdrop-blur-xl flex flex-col items-center justify-center gap-8 font-heading text-xl uppercase tracking-widest text-primary z-[160]">
          ...
          <button onClick={() => scrollTo('protocol')}>Маршруты</button>
          <button onClick={() => scrollTo('gallery')}>Галерея</button>
          <button onClick={() => scrollTo('services')}>Услуги</button>
          <div className="w-full h-px bg-white/10 my-4"></div>
          <button onClick={() => window.open('https://t.me/sparta_nakhodka', '_blank')} className="bg-accent text-white px-6 py-3 rounded-full uppercase tracking-widest font-bold text-sm">Написать в Telegram</button>
...
const Hero = () => (
  <section id="hero" className="relative h-[100dvh] w-full flex flex-col justify-center items-center text-center px-6 overflow-hidden bg-dark section-stack">
    ...
    <div className="relative z-10 max-w-5xl pt-32">
      <div className="font-heading font-bold text-accent tracking-[0.4em] uppercase mb-8 text-xs md:text-base">Интеллектуальный яхтинг в Приморье</div>
      <h1 className="font-drama italic text-[clamp(2.5rem,15vw,9.5rem)] text-white leading-[0.85] mb-12 break-words uppercase text-contour">Ветер и смыслы.</h1>
      <p className="font-data text-primary/70 max-w-2xl mx-auto uppercase text-xs md:text-sm tracking-widest leading-loose mb-16 px-4">Авторские маршруты под руководством легендарного шкипера. История, природа и паруса.</p>
      <MagneticButton className="bg-accent text-white px-10 md:px-14 py-5 md:py-6 text-xs md:text-sm font-heading font-bold uppercase tracking-widest shadow-2xl" onClick={() => window.open('https://wa.me/79147147468', '_blank')}>забронировать выход</MagneticButton>
    </div>
  </section>
);
...
const Features = () => {
  ...
  const text = "> ШКИПЕР СЕМЕН РЕМОВИЧ\n> СТАТУС: МАСТЕР СПОРТА\n> ОПЫТ: ВСЕ ОКЕАНЫ ПЛАНЕТЫ...";
    ...
    <section className="py-12 md:py-24 px-6 md:px-16 bg-background relative z-10 border-b border-dark/5">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10">
        ...
        <div onClick={() => document.getElementById('yacht').scrollIntoView({ behavior: 'smooth' })} className="bg-primary p-8 md:p-10 rounded-[2.5rem] h-72 md:h-80 relative overflow-hidden group cursor-pointer shadow-xl transition-all font-medium">
          ...
          <div className="font-data text-xs md:text-[10px] text-dark/60 uppercase tracking-widest leading-loose">Sprint 95 / France / 32 фута<br />Вместимость: 6 гостей<br />Гальюн / Душ / Камбуз</div>
...
const CaptainSection = () => (
  <section id="captain" className="relative py-24 md:py-40 px-6 md:px-16 bg-dark text-white min-h-[100dvh] flex flex-col md:flex-row gap-12 md:gap-20 items-center section-stack border-t border-white/5">
    <div className="w-full md:w-1/2 rounded-[2.5rem] md:rounded-[3.5rem] overflow-hidden aspect-[4/5] shadow-2xl relative img-container">
      <img src={captainImg} className="w-full h-full object-cover transition-all duration-1000" alt="Шкипер" />
    </div>
    <div className="w-full md:w-1/2 text-left text-white font-medium">
      ...
      <p className="font-data text-sm md:text-base text-white/80 uppercase tracking-widest leading-loose mb-12 max-w-lg">
        ...
        <br /><br />
        Его ценят за <span className="text-accent">уникальную историческую экспертизу</span> и энциклопедические знания. Вы получите не просто прогулку, а погружение в историю флота и морские байки, которые невозможно найти в книгах.
      </p>
      <blockquote className="border-l-4 border-accent pl-8 md:pl-10 italic font-drama text-xl md:text-3xl text-white leading-tight">«Он не просто катает — он учит чувствовать море и управлять парусами.»</blockquote>
    </div>
  </section>
);
...
const YachtSection = () => (
  <section id="yacht" className="relative py-24 md:py-40 px-6 md:px-16 bg-background text-dark min-h-[100dvh] flex flex-col md:flex-row-reverse gap-12 md:gap-20 items-center section-stack border-t border-dark/5">
    <div className="w-full md:w-1/2 rounded-[2.5rem] md:rounded-[3.5rem] overflow-hidden aspect-video shadow-2xl img-container">
      <img src={yachtImg} className="w-full h-full object-cover" alt="Яхта SPARTA" />
    </div>
    <div className="w-full md:w-1/2">
      <div className="font-heading font-bold text-accent uppercase tracking-[0.4em] mb-10 text-xs text-left font-medium uppercase tracking-[0.2em]">французская классика</div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-6 md:gap-10 text-left">
        <div><div className="font-data text-xs md:text-[10px] text-dark/30 uppercase mb-2 tracking-widest">Проект</div><div className="font-heading font-bold text-lg md:text-2xl uppercase tracking-tighter text-dark font-medium">Sprint 95</div></div>
        <div><div className="font-data text-xs md:text-[10px] text-dark/30 uppercase mb-2 tracking-widest">Длина</div><div className="font-heading font-bold text-lg md:text-2xl uppercase tracking-tighter text-dark font-medium">32 Фута</div></div>
        <div><div className="font-data text-xs md:text-[10px] text-dark/30 uppercase mb-2 tracking-widest">Верфь</div><div className="font-heading font-bold text-lg md:text-2xl uppercase tracking-tighter text-dark font-medium">Archambault</div></div>
        <div><div className="font-data text-xs md:text-[10px] text-dark/30 uppercase mb-2 tracking-widest">Год</div><div className="font-heading font-bold text-lg md:text-2xl uppercase tracking-tighter text-dark font-medium">1992</div></div>
        <div><div className="font-data text-xs md:text-[10px] text-dark/30 uppercase mb-2 tracking-widest">Гости</div><div className="font-heading font-bold text-lg md:text-2xl uppercase tracking-tighter text-dark font-medium">6 Чел</div></div>
        <div><div className="font-data text-xs md:text-[10px] text-dark/30 uppercase mb-2 tracking-widest">Статус</div><div className="font-heading font-bold text-lg md:text-2xl uppercase tracking-tighter text-dark font-medium">Победитель</div></div>
      </div>
      <p className="mt-12 font-data text-xs md:text-xs text-dark uppercase tracking-widest leading-loose text-left max-w-xl font-medium">
        Построена во Франции. Элегантная и быстрая, она создана для тех, кто ценит эстетику настоящего парусного спорта. На борту: камбуз, гальюн и душ на палубе.
        <br /><br />
        <span className="italic text-dark/60">32 фута — это идеальный размер, чтобы чувствовать динамику волны, сохраняя комфорт частного лофта.</span>
      </p>
    </div>
  </section>
);
...
const ProtocolCards = () => {
  const routes = [
    { num: '01', title: 'Остров Аскольд', desc: 'Маяк, заброшенные базы и дикие олени. Слушайте историю открытий Приморья.', img: protocolImg1 },
    { num: '02', title: 'Остров Лисий', desc: 'Заповедная жемчужина залива Находка. Тишина и чистое море.', img: protocolImg2 },
    { num: '03', title: 'Трёхозёрье', desc: 'Бирюзовая вода и белоснежный песок. Визитная карточка нашего края.', img: protocolImg3 },
  ];
        ...
        <section key={i} className="h-[100dvh] w-full flex items-center justify-center bg-background overflow-hidden border-t border-dark/5 section-stack">
          <div className="bg-primary w-full h-full flex flex-col md:flex-row relative">
            <div className="w-full md:w-1/2 h-1/2 md:h-full overflow-hidden shrink-0 img-container">
              <img src={r.img} className="w-full h-full object-cover transition-all duration-1000" alt={r.title} />
            </div>
            <div className="w-full md:w-1/2 h-1/2 md:h-full p-8 md:p-24 flex flex-col justify-center bg-primary text-left">
              <div className="font-data text-accent text-2xl mb-6 tracking-[0.5em] uppercase font-medium">маршрут {r.num}</div>
              <h2 className="font-drama italic text-[clamp(2.5rem,10vw,7.5rem)] text-dark mb-10 leading-[0.9] break-words uppercase">{r.title}</h2>
  ...
  return (
    <section id="gallery" className="relative py-24 md:py-40 px-6 md:px-16 bg-dark flex flex-col justify-center border-t border-white/5">
      <div className="max-w-7xl mx-auto w-full">
        <h2 className="font-drama italic text-[clamp(2rem,10vw,8rem)] mb-16 md:mb-24 text-center leading-none uppercase text-white">морские моменты.</h2>
...
const Services = () => (
  <section id="services" className="relative py-12 md:py-20 px-6 md:px-16 bg-background border-t border-dark/5">
    <div className="max-w-7xl mx-auto w-full text-center">
      <h2 className="font-drama italic text-[clamp(2rem,8vw,6rem)] text-dark mb-8 md:mb-12 leading-none uppercase tracking-tighter">форматы.</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        {[
          { title: "обучение", items: ["Работа с парусами", "Навигация", "Основы яхтинга"], icon: <Waves className="text-accent" /> },
          { title: "регаты", items: ["Гоночный экипаж", "Спортивная техника", "Командный дух"], icon: <Heart className="text-accent" />, featured: true },
          { title: "экспертиза", items: ["Морские байки", "История открытий", "Краеведение"], icon: <Fish className="text-accent" /> }
        ].map((s, i) => (
          <div key={i} className={`p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] border transition-all flex flex-col ${s.featured ? 'bg-dark text-white border-accent shadow-xl scale-100 md:scale-105 z-10' : 'bg-primary border-dark/5 text-dark'}`}>
            ...
            <MagneticButton className={`w-full py-3 md:py-4 px-4 text-xs md:text-[10px] font-bold uppercase tracking-widest shadow-lg ${s.featured ? 'bg-accent text-white' : 'bg-dark text-white'}`} onClick={() => window.open('https://wa.me/79147147468', '_blank')}>забронировать</MagneticButton>
          </div>
        ))}
      </div>
    </div>
...
const RulesSection = () => (
  <section className="py-20 px-6 md:px-16 bg-dark text-white border-t border-white/5">
    <div className="max-w-4xl mx-auto text-left">
      <h3 className="font-heading font-bold text-accent uppercase tracking-widest text-xs mb-10">Морской этикет</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        <div>
          <h4 className="font-drama italic text-2xl mb-6 text-white uppercase">Что взять с собой:</h4>
          <ul className="font-data text-xs uppercase tracking-widest space-y-4 text-white/60">
            <li>• Чистая обувь на светлой подошве</li>
            <li>• Солнцезащитные очки и крем</li>
            <li>• Ветровка (в море всегда прохладнее)</li>
            <li>• Купальные принадлежности</li>
          </ul>
        </div>
        <div>
          <h4 className="font-drama italic text-2xl mb-6 text-white uppercase">На борту не приветствуется:</h4>
          <ul className="font-data text-xs uppercase tracking-widest space-y-4 text-white/60">
            <li>• Обувь с черной подошвой</li>
      ...
      layers.forEach((layer) => {
        ScrollTrigger.create({
          ...
          onUpdate: (self) => {
            ...
            if (img) {
              // УБРАЛИ ГРЕЙСКЕЙЛ (ЧБ), оставили только прозрачность и масштаб
